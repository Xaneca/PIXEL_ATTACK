# Original code adapted from https://github.com/Hyperparticle/one-pixel-attack-keras/blob/master/helper.py
# CIFAR - 10
...
def evaluate_models(models, modelNames, x_test, y_test):
    correct_imgs = []
    network_stats = []
    i = 0
    for model in models:
        ...
        correct = [[modelName, i, label, np.max(pred), pred]
                   for i, (label, pred)
                   in enumerate(zip(y_test[:, 0], probabilities))
                   if label == np.argmax(pred)]
        accuracy = len(correct) / len(x_test)

        correct_imgs += correct
        network_stats += [[modelName, accuracy]]
        i = i + 1
