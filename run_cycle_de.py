...
# Helper functions
from differential_evolution import differential_evolution_no_cx
import helper
import gc
...
### SHORT VERSION OF ALGORITHM - CHANGE SOME PARAMETERS: ###
nruns = 5
seeds = list(range(nruns))
n_samples = 2
pop_size = 400
...
for model in models:
    ...
    # Create de folder
    de_folder = f'./results/{modelName}/de'
    if not os.path.exists(de_folder):
        os.makedirs(de_folder)

    # Time
    time_file = f'{de_folder}/time.csv'
    ...
    images_idx = [7760, 1129] # changed ⚠
    images_labels = images_df['true label']

    # Write csv with metrics
    file_metrics = f'{de_folder}/metrics.csv'
    ...
    file_cover = f'{de_folder}/covered_pixels.csv'
    ...
    for run in range(1, nruns+1):
        ...
        run_folder = f'{de_folder}/run_{run}'
        ...
        for i in range(n_samples):
            ...
            best_fit, avg_fit, best_ind, suc, suc_act_total, n_covered_pixels = differential_evolution_no_cx(img, label, model, pop_size, generations, mut, bounds, img_folder, seeds[run-1])
            ...
            writer_npixels.writerow(['de', run, img_idx, i, suc])
            f_npixels.close()
            ...
            # Save img metrics
            folder_metrics_img = f'{de_folder}/metrics_img'
    ...
    writer_means.writerow(['de', mean_suc_rate_dataset])
    f_means.close()

    mn += 1

    # Specify the file path for the CSV
    csv_file = f'{de_folder}/parameters.csv'

    # Create a list of parameter names and their corresponding values
    parameter_names = ["nruns", "seeds", "n_samples", "samples", "pop_size", "generations", "pixels", "mut"]
    parameter_values = [nruns, seeds, n_samples, samples, pop_size, generations, pixels, mut]
    ...
    for img_idx, success_rate in dict_success_rate.items():
        ...
        writer_metrics_img.writerow([counter, img_idx, 'de', mean_success_rate, [mean_adv_prob1, mean_adv_prob2]])
        counter += 1
    f_metrics_img.close()